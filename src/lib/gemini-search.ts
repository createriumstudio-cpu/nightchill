...
必ずJSONのみを出力してください。マークダウンのコードブロック（\`\`\`json ... \`\`\`）は使わないでください。説明文も不要です。`;
    ...
    if (!jsonMatch) {
      console.warn(`[gemini-search] Could not parse response for: ${query}, text: ${text.slice(0, 200)}`);
      return createFallbackVenue(query, area);
    }
...
必ずJSON配列のみを出力してください。マークダウンのコードブロック（\`\`\`json ... \`\`\`）は使わないでください。説明文も不要です。`;
    ...
    if (jsonArrayMatch) {
      ...
      // JSON配列が見つからない場合、個別の JSON オブジェクトを抽出して配列化
      console.warn(`[gemini-search] Could not find JSON array in batch response, text: ${text.slice(0, 200)}`);
      const objectMatches = jsonText.match(/\{[^{}]*"name"[^{}]*\}/g);
      if (objectMatches && objectMatches.length > 0) {
        console.log(`[gemini-search] Found ${objectMatches.length} individual JSON objects, assembling array`);
        try {
          parsed = objectMatches.map((m) => JSON.parse(m));
        } catch {
          console.warn("[gemini-search] Failed to parse individual objects, falling back to individual search");
          return await fallbackToIndividualSearch(venues, area, resultMap);
        }
      } else {
        console.warn("[gemini-search] No JSON found in batch response, falling back to individual search");
        return await fallbackToIndividualSearch(venues, area, resultMap);
      }
    }
...
必ずJSON配列のみを出力してください。マークダウンのコードブロック（\`\`\`json ... \`\`\`）は使わないでください。説明文も不要です。`;
    ...
    const jsonMatch = jsonText.match(/\[[\s\S]*\]/);
    if (!jsonMatch) {
      console.warn(`[gemini-search] Could not parse trending spots response, text: ${text.slice(0, 200)}`);
      return [];
    }
