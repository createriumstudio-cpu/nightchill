...
【デートの種類に応じたルール】
- 「食事のみ」→ 駅から近いレストランを中心に、2〜3時間のコンパクトなプラン
- 「半日デート」→ 4〜5時間程度、食事+1アクティビティ
- 「終日デート」→ 6〜8時間、複数のスポットを巡るプラン
- 「お泊まりデート」→ 翌日の朝食まで含め、ホテルの提案も行う

【年齢制限ルール - 厳守】
- 「20歳未満」の場合: バー、居酒屋、シーシャ、ナイトクラブなど、アルコールやタバコを提供する店舗は絶対に推薦してはならない。カフェ、レストラン、アミューズメント施設など年齢制限のない場所のみ提案する
- 「20歳以上」の場合: 全ての店舗を提案可能。バーやシーシャラウンジも含めてよい

【天気情報の活用ルール】
- 天気情報が提供されている場合、必ずプランに反映する
- 雨の場合: 屋内のスポットを中心にプランを組み、傘の準備をwarningsに含める
- 猛暑の場合: 涼しい屋内施設を優先し、水分補給のtipを含める
- 寒い場合: 暖かいカフェやレストラン中心に、防寒対策をfashionAdviceに反映
- 晴れの場合: テラス席や公園散歩も積極的に取り入れる
- 天気に関する具体的なアドバイスをtimelineのtipやwarningsに含める
...
function buildUserPrompt(
  ...
  const parts = [
    ...
    `デートの種類: ${dateTypeLabels[request.dateType]}`,
...
export async function generateAIPlan(request: PlanRequest): Promise<DatePlan> {
  ...
  const isOvernight = request.dateType === "overnight";
